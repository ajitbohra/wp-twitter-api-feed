...
function taf_load_plugin_textdomain() {
	...
	$locale = apply_filters( 'plugin_locale', get_locale(), 'TAF' );
	// wp-content/languages/plugin-name/plugin-name-en_EN.mo.
	load_textdomain( 'TAF', trailingslashit( WP_LANG_DIR ) . 'TAF' . '/' . 'TAF' . '-' . $locale . '.mo' );
	// wp-content/plugins/plugin-name/languages/plugin-name-en_EN.mo.
	load_plugin_textdomain( 'TAF', false, basename( TAF_PLUGIN_DIR ) . '/languages/' );
}
...
if ( $taf_widget ) {
	require_once TAF_PLUGIN_DIR . 'includes/class-taf-widget.php';
}

/**
 * Add plugin setting link to plugin row meta
 *
 * @param array  $links meta row links array.
 * @param string $file plugin basename.
 *
 * @return array final links array for meta row
 */
